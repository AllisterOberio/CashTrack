...
public class AddRentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dateEditText = findViewById(R.id.rent_date_input);
        amountEditText = findViewById(R.id.rent_price_input);
        expenseTitleEditText = findViewById(R.id.rent_details_input);
        addButton = findViewById(R.id.btn_save_rent);

        // Set up the date picker
        dateEditText.setOnClickListener(v -> showDatePicker());

        // Set up the "Add Rent" button click listener
    ...
    private void saveRentExpense() throws JSONException {
        ...
        try {
            ...
            // Retrieve stored rent records
            SharedPreferences prefs = getSharedPreferences("FinanceData", MODE_PRIVATE);
            String existingData = prefs.getString("rentRecords", "[]");

            // Convert to JSONArray and add new transaction
            JSONArray rentArray = new JSONArray(existingData);
            ...
            rentArray.put(newTransaction);

            // Save updated array back to SharedPreferences
            SharedPreferences.Editor editor = prefs.edit();
            editor.putString("rentRecords", rentArray.toString());
            editor.apply();

            // Pass data and navigate
            Intent intent = new Intent(AddRentActivity.this, RentRedirectActivity.class);
            intent.putExtra("rentRecords", rentArray.toString());
            startActivity(intent);
            finish();

        } catch (NumberFormatException | JSONException e) {
            Toast.makeText(this, "Error saving rent data", Toast.LENGTH_SHORT).show();
        }
    }
