...
public class AddGiftsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dateEditText = findViewById(R.id.gifts_date_input);
        amountEditText = findViewById(R.id.gifts_price_input);
        expenseTitleEditText = findViewById(R.id.gifts_details_input);
        addButton = findViewById(R.id.btn_save_gifts);

        // Set up the date picker
        dateEditText.setOnClickListener(v -> showDatePicker());

        // Set up the "Add Gifts" button click listener
    ...
    private void saveGiftsExpense() throws JSONException {
        ...
        try {
            ...
            // Retrieve stored gifts records
            SharedPreferences prefs = getSharedPreferences("FinanceData", MODE_PRIVATE);
            String existingData = prefs.getString("giftsRecords", "[]");

            // Convert to JSONArray and add new transaction
            JSONArray giftsArray = new JSONArray(existingData);
            ...
            giftsArray.put(newTransaction);

            // Save updated array back to SharedPreferences
            SharedPreferences.Editor editor = prefs.edit();
            editor.putString("giftsRecords", giftsArray.toString());
            editor.apply();

            // Pass data and navigate
            Intent intent = new Intent(AddGiftsActivity.this, GiftsRedirectActivity.class);
            intent.putExtra("giftsRecords", giftsArray.toString());
            startActivity(intent);
            finish();

        } catch (NumberFormatException | JSONException e) {
            Toast.makeText(this, "Error saving gifts data", Toast.LENGTH_SHORT).show();
        }
    }
