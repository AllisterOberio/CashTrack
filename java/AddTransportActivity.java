...
public class AddTransportActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dateEditText = findViewById(R.id.transport_date_input);
        amountEditText = findViewById(R.id.transport_price_input);
        expenseTitleEditText = findViewById(R.id.transport_details_input);
        addButton = findViewById(R.id.btn_save_transport);

        // Set up the date picker
        dateEditText.setOnClickListener(v -> showDatePicker());

        // Set up the "Add Transport" button click listener
    ...
    private void saveTransportExpense() throws JSONException {
        ...
        try {
            ...
            // Retrieve stored transport records
            SharedPreferences prefs = getSharedPreferences("FinanceData", MODE_PRIVATE);
            String existingData = prefs.getString("transportRecords", "[]");

            // Convert to JSONArray and add new transaction
            JSONArray transportArray = new JSONArray(existingData);
            ...
            transportArray.put(newTransaction);

            // Save updated array back to SharedPreferences
            SharedPreferences.Editor editor = prefs.edit();
            editor.putString("transportRecords", transportArray.toString());
            editor.apply();

            // Pass data and navigate
            Intent intent = new Intent(AddTransportActivity.this, TransportRedirectActivity.class);
            intent.putExtra("transportRecords", transportArray.toString());
            startActivity(intent);
            finish();

        } catch (NumberFormatException | JSONException e) {
            Toast.makeText(this, "Error saving transport data", Toast.LENGTH_SHORT).show();
        }
    }
