...
public class AddGroceriesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dateEditText = findViewById(R.id.groceries_date_input);
        amountEditText = findViewById(R.id.groceries_price_input);
        expenseTitleEditText = findViewById(R.id.groceries_details_input);
        addButton = findViewById(R.id.btn_save_groceries);

        // Set up the date picker
        dateEditText.setOnClickListener(v -> showDatePicker());

        // Set up the "Add Groceries" button click listener
        addButton.setOnClickListener(v -> {
            try {
                saveGroceriesExpense();
    ...
    private void saveGroceriesExpense() throws JSONException {
        ...
        try {
            ...
            // Retrieve stored groceries records
            SharedPreferences prefs = getSharedPreferences("FinanceData", MODE_PRIVATE);
            String existingData = prefs.getString("groceriesRecords", "[]");

            // Convert to JSONArray and add new transaction
            JSONArray groceriesArray = new JSONArray(existingData);
            ...
            groceriesArray.put(newTransaction);

            // Save updated array back to SharedPreferences
            SharedPreferences.Editor editor = prefs.edit();
            editor.putString("groceriesRecords", groceriesArray.toString());
            editor.apply();

            // Pass data and navigate
            Intent intent = new Intent(AddGroceriesActivity.this, GroceriesRedirectActivity.class);
            intent.putExtra("groceriesRecords", groceriesArray.toString());
            startActivity(intent);
            finish();

        } catch (NumberFormatException | JSONException e) {
            Toast.makeText(this, "Error saving groceries data", Toast.LENGTH_SHORT).show();
        }
    }
